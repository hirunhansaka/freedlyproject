<?php
// freelancer_view_applications.php
...
$stmt = $pdo->prepare("SELECT p.status AS proposal_status, j.id AS job_id, j.title, j.budget FROM proposals p JOIN jobs j ON p.job_id = j.id WHERE p.freelancer_id = ? ORDER BY p.submitted_at DESC");
...
<head>
    ...
    <title>My Job Applications - Freedly</title>
...
.sidebar-menu ul li a.active .icon {
    color: var(--primary-blue-dark);
        ...
        /* ✅ NEW: Application List & Card Styles */
        .application-list {
            display: grid;
            gap: 1.5rem;
        }
        ...
        .application-card .budget {
            color: var(--light-text);
            margin: 0.25rem 0 0 0;
            font-size: 1rem;
            font-weight: 500;
        }
        .application-card .budget span {
            font-weight: 700;
    ...
    <div class="dashboard-wrapper">
        <aside class="sidebar">
            ...
            <div class="sidebar-menu">
                ...
                <ul>
                    ...
                    <li><a href="freelancer_view_applications.php" class="active"><i class="fas fa-file-alt icon"></i> Job Applications</a></li>
                </ul>
                <h3>Services</h3>
                 <ul>
        ...
        <main class="main-content-wrapper">
            <div class="main-content-header">
                ...
                <p>Track the status of all the job proposals you have submitted.</p>
            </div>
            ...
            <div class="application-list">
                <?php if (count($applications) > 0): ?>
                    ...
                    foreach ($applications as $app): 
                        ...
                        $status_class = '';
                        if ($app['proposal_status'] == 'pending') {
                            $status_class = 'status-pending';
                        } elseif ($app['proposal_status'] == 'accepted') {
                            $status_class = 'status-accepted';
                        } elseif ($app['proposal_status'] == 'rejected') {
                            $status_class = 'status-rejected';
                        }
                    ?>
                        <div class="application-card" style="animation-delay: <?= $delay ?>s;">
                            <div class="application-details">
                                <h3><?= htmlspecialchars($app['title']) ?></h3>
                                <p class="budget">Budget: <span>$<?= htmlspecialchars($app['budget']) ?></span></p>
                            </div>
                            <div class="application-status">
                                <span class="status-badge <?= $status_class ?>">
                                    <?= htmlspecialchars($app['proposal_status']) ?>
                                </span>
                            </div>
                            <div class="application-action">
                                <?php if($app['proposal_status'] == 'accepted'): ?>
                                    <a href="freelancer_my_projects.php" class="btn btn-primary">Go to Projects</a>
                                <?php else: ?>
                                    <a href="apply_to_job.php?job_id=<?= $app['job_id'] ?>" class="btn btn-secondary">View Job</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <div class="application-card" style="animation-delay: 0.1s;">
                        <p>You have not applied to any jobs yet. <a href="index.php#jobs">Find work now!</a></p>
                    </div>
                <?php endif; ?>
            </div>
        </main>
